-- Код предоставленный преподавателем(исходник)
create table users
(
...
SELECT COLOR, COUNT(COLOR) AS QUANTITY, MARK FROM auto
WHERE MARK = 'BMW'
GROUP BY COLOR
UNION SELECT COLOR, COUNT(COLOR) AS QUANTITY, MARK FROM auto
WHERE MARK = 'LADA'
GROUP BY COLOR;
...
WHERE MARK = 'BMW'
UNION SELECT COUNT(MARK) AS OTHER_QUANTITY FROM auto
WHERE MARK NOT IN ('BMW');
...
WHERE MARK = 'VOLVO'
UNION SELECT COUNT(MARK) AS OTHER_QUANTITY FROM auto
WHERE MARK NOT IN ('VOLVO');

-- Задание №3. Даны 2 таблицы, созданные следующим образом:
create table test_a (id int, data varchar(12));
